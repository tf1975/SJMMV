...
function bind(){
 ...
 $('#modeBtn').onclick=()=>{state.profile.mode=state.profile.mode==='reread'?'first':'reread';save();renderAll()};
...
function renderAll(){$('#readerBtn').textContent=state.profile.name;$('#settingsMode').value=state.profile.mode;$('#modeStatus').textContent=state.profile.mode==='reread'?'Reread Mode — future significance appears only after its payoff is safely known.':'First Read — future context remains hidden.';document.body.classList.toggle('reread',state.profile.mode==='reread');$('#notice').textContent=catalog.platform.notice;renderReleases();renderLibrary();renderReaders();renderMentions();renderLore('all');renderReaderList()}
...
function renderChapter(){
 ...
 $('#art').src=seed?.image||'assets/chapter-art/tog/chapter-01.svg';$('#art').alt=seed?`Original spoiler-safe artwork for ${currentBook.title}, Chapter ${currentChapter}`:'Original chapter artwork placeholder';
