...
function showAuthPanel(id){['authHome','emailLoginStep','emailSignupStep','emailConfirmationStep','passwordResetStep','passwordUpdateStep'].forEach(panelId=>{const panel=$('#'+panelId);if(panel)panel.hidden=panelId!==id});$('#authMessage').textContent='';$('#signupNicknameMessage').textContent=''}
